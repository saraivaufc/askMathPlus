...
class StudentLessonState(models.Model):
	student = models.ForeignKey('Person', verbose_name=_(u"Person"))
	...
	answered_correct_questions = models.ManyToManyField('Question', related_name="Answered Correct Questions",
														verbose_name=_(u"Answered Questions"), null=True, blank=True)
	answered_incorrect_questions = models.ManyToManyField('Question', related_name="Answered Incorrect Questions",
														  verbose_name=_(u"Answered Questions"), null=True, blank=True)
	skipped_questions = models.ManyToManyField('Question', related_name="Skipped Questions",
											   verbose_name=_(u"Skipped Questions"), null=True, blank=True)
	help_questions = models.ManyToManyField('Question', related_name="Help Questions",
											verbose_name=_(u"Help Questions"), null=True, blank=True)
	...
	def get_question(self, last_question=None, definitive=False):
		...
		questions_remaining = (
		((questions_all.difference(answered_correct_questions)).difference(answered_incorrect_questions)).difference(
			skipped_questions)).difference(last_question_list);
	...
	def update_percentage_completed(self):
		try:
			self.percentage_completed = (len(self.get_answered_correct_questions()) * 100) / len(
				self.lesson.get_questions_visibles())
